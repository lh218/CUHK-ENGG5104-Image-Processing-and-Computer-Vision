...
if __name__ == '__main__':
    ...
    parser.add_argument('--total_epochs', type=int, default=51)
    ...
    parser.add_argument('--crop_size', type=int, nargs='+', default=[192, 256],
    ...
    tools.add_arguments_for_module(parser, models, argument_for_class='model', default='FlowNetOurs')

    tools.add_arguments_for_module(parser, losses, argument_for_class='loss', default='OursLoss')

    tools.add_arguments_for_module(parser, torch.optim, argument_for_class='optimizer', default='Adam',
                                   skip_params=['params'])

    tools.add_arguments_for_module(parser, datasets, argument_for_class='training_dataset', default='MpiSintelClean',
                                   skip_params=['is_cropped'],
                                   parameter_defaults={'root': '../data/training'})

    tools.add_arguments_for_module(parser, datasets, argument_for_class='validation_dataset', default='MpiSintelClean',
                                   skip_params=['is_cropped'],
                                   parameter_defaults={'root': '../data/validation',
                                                       'replicates': 1})
    ...
    with tools.TimerBlock("Initializing Datasets") as block:
        ...
        if exists(args.validation_dataset_root):
            ...
            validation_dataset = args.validation_dataset_class(args, False,
    ...
    for epoch in range(args.start_epoch, args.total_epochs + 1):
        ...
        if not args.skip_training:
            train_loss, iterations = train(args=args, epoch=epoch, start_iteration=global_iteration,
                                           ...
                                           model=model_and_loss, optimizer=optimizer, logger=train_logger,
                                           offset=offset)
        ...
        if not args.skip_validation and ((epoch - 1) % args.validation_frequency == 0
            ...
            validation_loss, _ = train(args=args, epoch=epoch - 1, start_iteration=global_iteration,
                                       ...
                                       model=model_and_loss, optimizer=optimizer, logger=validation_logger,
                                       is_validate=True, offset=offset)
            offset += 1
