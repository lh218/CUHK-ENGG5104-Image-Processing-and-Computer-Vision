...
def show_correspondence(imgA, imgB, X1, Y1, X2, Y2, file_name='result'):
    ...
    for i in range(X1.shape[0]):
        ...
        newImg = cv2.circle(newImg, (int(X1[i]), int(Y1[i])), 9, cur_color, -1)
        newImg = cv2.circle(newImg, (int(X1[i]), int(Y1[i])), 9, [0, 0, 0], 2)
        newImg = cv2.circle(newImg, (int(X2[i]+shiftX), int(Y2[i])), 9, cur_color, -1)
        newImg = cv2.circle(newImg, (int(X2[i]+shiftX), int(Y2[i])), 9, [0, 0, 0], 2)
...
def show_correspondence2(imgA, imgB, X1, Y1, X2, Y2, file_name='result'):
    ...
    for i in range(X1.shape[0]):
        cur_color = np.random.rand(3)
        newImg = cv2.circle(newImg, (int(X1[i]), int(Y1[i])), 5, cur_color, -1)
        newImg = cv2.circle(newImg, (int(X2[i]+shiftX), int(Y2[i])), 5, cur_color, -1)
        newImg = cv2.line(newImg, (int(X1[i]), int(Y1[i])), (int(X2[i]+shiftX), int(Y2[i])), cur_color, 2)
